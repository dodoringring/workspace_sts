...
const KakaoRedirectHandler = () => {
  //카카오 객체를 global variable에 등록해주는 코드임 window객체에 Kakao가 등록이 되어있어야함...
  //const {Kakao} = window;
  ...
  const getToken = async () => {
    ...
    try {
      ...
      navigate("/profile");//이렇게 오는 값들은 반드시 라우터에 등록이 되어있어야함.
    } catch(error) {
      console.log(error);
    }
  }
  ...
  return (
    <>
      {/* 아무런 의미없는 화면이다. - 거쳐서 다른 화면으로 이동하니까. - 루트컨텍스트 - 인증이 되면 /home으로 가자. route에 설정된 홈 */}
      {code}
    </>
  )
}
